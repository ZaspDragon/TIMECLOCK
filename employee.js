...
import { tzPrettyTime, tzPrettyDate, tzDateISO, tzTimeHHMMSS, dayNameFromISO, calcLunchMins, calcTotalHours, startOfWeekISO, addDaysISO } from "./common.js";
...
function exportXLSX(){
  ...
  myRows.forEach(r=>{
    aoa.push([r.estDate, r.day, identity.name, identity.company, r.clockIn||"", r.lunchOut||"", r.endLunch||"", r.clockOut||"", calcLunchMins(r)===""?"":calcLunchMins(r), calcTotalHours(r)===""?"":calcTotalHours(r)]);
  });
...
function init(){
  ...
  if(identity){
    ...
    els.pin.value = identity.pin || "";
  }

  // The real PIN validity is checked server-side.
  // When user saves identity, we run a quick query (0 rows is fine) to validate:
  // - approved employee
  // - current rotating PIN
  ...
  els.identityForm.addEventListener("submit", async (e)=>{
    ...
    if(!name || !company || !pin){ setMsg("Fill Name, Company, and PIN."); return; }
    identity = { name, company, pin };
  ...
  // If identity exists, try validate
