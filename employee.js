...
// Identity + state
// -------------------------
const ID_KEY = "tc_identity_v3"; // keep key; now stores {name, company, date?}
let identity = null;
let myRows = [];
...
function renderToday(row) {
  ...
  // Button enablement based on today's row state
...
// -------------------------
// Enable / disable stamp buttons as a group
// -------------------------
function enableButtons(on) {
  // IMPORTANT: force these buttons to never submit any form
...
async function stampField(field) {
  if (!identity) {
    setMsg("Fill sign-in first.");
    return;
  }

  // hard guard so buttons always work only after enabling
  if ([els.clockIn, els.lunchOut, els.endLunch, els.clockOut].some((b) => b.disabled)) {
    // If user didn't hit "Save & Enable Buttons"
    // allow stamping if identity exists AND validation already passed.
    // We'll just try—if the backend rejects, user sees why.
  }
...
function init() {
  ...
  // Default history range (this week)
  ensureDefaultRange();

  // Disabled until identity saved + validated
  enableButtons(false);

  // Load saved identity (no PIN)
  ...
  els.identityForm.addEventListener("submit", async (e) => {
    ...
    // optionally store the selected date too (useful later)
  ...
  // Stamp buttons (force type=button already handled in enableButtons)
  ...
  // Auto enable if identity exists
